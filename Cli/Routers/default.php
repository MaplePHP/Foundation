...
$routes->cli("/test", ['MaplePHP\Foundation\Cli\Connectors\Test', "test"]);

$routes->group("/server", function ($routes) {
    // It is recommended to add this handle at the beginning of every grouped call
    ...
    $routes->cli("/test", ['MaplePHP\Foundation\Cli\Connectors\Server', "test"]);
});
...
$routes->group("/make", function ($routes) {
    // It is recommended to add this 2 handles at the beginning of every grouped call
    $routes->map("*", '[/{any:.*}]', ['MaplePHP\Foundation\Cli\Connectors\Cli', "handleMissingType"]);
    $routes->cli("[/help]", ['MaplePHP\Foundation\Cli\Connectors\Make', "help"]);

    $routes->cli("/{action:[^/]+}", ['MaplePHP\Foundation\Cli\Connectors\Make', "make"]);
